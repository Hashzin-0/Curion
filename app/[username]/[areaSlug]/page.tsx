...
export default function AreaResume() {
  ...
  const handleExportThemed = async () => {
    ...
    setExporting(true);
    try {
      const res = await fetch('/api/resume/theme', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profession: area.name, name: user.name }) });
      const theme: ResumeTheme = await res.json();
      setExportTheme(theme);
      setExportData({
        name: user.name, firstName: user.name.split(' ')[0], lastName: user.name.split(' ').slice(1).join(' '),
        profession: area.name, phone: user.phone || '', email: user.email || '', availableSince: 'Hoje',
    ...
    } catch (err) { toast.error('Erro ao gerar PDF'); setExporting(false); }
  };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await html2pdf().set({ margin: 0, filename: `cv-${area?.slug}.pdf`, jsPDF: { format: [794, 1123], unit: 'px' } }).from(pdfRef.current).save();
      setShouldExport(false); setExporting(false);
    };
    run();
  }, [shouldExport, area]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b px-6 py-3 flex items-center justify-between">
        <Button variant="secondary" onClick={() => router.back()}><LucideIcons.ArrowLeft size={16} /> Voltar</Button>
        <div className="flex gap-2">
          {isOwner && (
            <>
              <Button variant={isEditMode ? 'primary' : 'outline'} onClick={() => setIsEditMode(!isEditMode)}>Modo Edição</Button>
      ...
      <div className="max-w-[800px] mx-auto my-12 bg-white shadow-2xl relative overflow-hidden p-12 border-t-[12px]" style={{ borderColor: area.theme_color || theme.hex }}>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-2xl font-black uppercase tracking-widest">Experiência</h2>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-2xl font-black uppercase tracking-widest">Formação</h2>
            <div className="flex-1 h-1 bg-slate-100" />
          </div>
        ...
        <footer className="mt-20 pt-12 border-t flex justify-between items-center">
