...
export function InterviewSimulatorModal({ isOpen, onClose, areaName }: Props) {
  ...
  const handleNativeSpeak = (text: string) => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!isMuted) {
        ...
        else handleNativeSpeak(result.text);
      }
    } catch (e) {
      ...
      <div className="flex flex-col h-[600px]">
        <div className="bg-blue-600/10 p-3 flex items-center justify-between border-b dark:border-slate-800">
           <div className="flex items-center gap-2">
             ...
             <span className="text-[10px] font-black uppercase text-blue-600 tracking-widest">TTS Local Ativo (ESPnet/Kokoro/Piper)</span>
           </div>
            ...
            {messages.map((msg, i) => (
              ...
              >
                <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`p-5 rounded-3xl text-sm font-medium shadow-sm relative ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-tl-none border border-slate-100 dark:border-slate-700'}`}>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-3xl rounded-tl-none flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-slate-500 uppercase tracking-widest animate-pulse">Processando Voz Local...</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="p-6 border-t dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <div className="flex justify-center mt-4">
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
               ...
               Voz premium sem custo via motores open source locais
             </p>
          </div>
        </div>
      </div>
