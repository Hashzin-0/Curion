...
export function AnalyticsDashboard({ userId }: { userId: string }) {
  ...
  if (loading) return <div className="h-64 flex items-center justify-center bg-white dark:bg-slate-900 rounded-[2.5rem] animate-pulse">Carregando métricas...</div>;
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="text-3xl font-black text-slate-900 dark:text-white">{chartData.reduce((acc, d) => acc + d.views, 0)}</div>
        </div>
      </div>
