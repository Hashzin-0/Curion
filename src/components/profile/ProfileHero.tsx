...
export function ProfileHero({ user, theme, isOwner, onEdit, accentColor, darkColor, areas = [], qrCodeLogoUrl }: Props) {
  ...
  return (
    <>
      ...
      <DigitalWalletPass user={{ ...user, headline: user.headline || undefined, avatar_path: user.avatar_path || undefined }} isOpen={isWalletOpen} onClose={() => setIsWalletOpen(false)} accentColor={accentColor} logoUrl={qrCodeLogoUrl} />
    </>
  );
}
